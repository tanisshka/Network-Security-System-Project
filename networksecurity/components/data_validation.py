...
class DataValidation:
    ...
    def dataset_drift_report(self,base_df,current_df,threshold=0.05)->bool:
        try:
            status=True
            report={}
    ...
    def initaite_data_validation(self)->DataingestionArtifact:
        try:
            ...
            status_train=self.validate_no_of_columns(dataframe=train_dataframe)
            if not status_train:
                raise Exception("Train dataframe does not contain all the columns")
                
            status_test=self.validate_no_of_columns(dataframe=test_dataframe)
            if not status_test:
                raise Exception("Test dataframe does not contain all the columns")
                
            status=self.dataset_drift_report(base_df=train_dataframe,current_df=test_dataframe)
